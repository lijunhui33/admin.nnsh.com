	...
	<div :style="!childIsCollapse ? 'width: 220px;' : ''">
		<a-menu mode="inline" theme="dark" :inline-collapsed="childIsCollapse" @click="menuClick" :selectedKeys="[selectedKeys]" :defaultOpenKeys="defaultOpenKeys">
			<a-sub-menu v-for="child in route" :key="child.name" v-if="child.meta.hasSubMenu">
				<span slot="title" :style="child.meta.left?'margin-left:10px':''">
					<a-icon :type="child.meta.icon" />
					<span>{{ child.meta.title }}</span>
				</span>
				<MenuItem :route="child.children" :isCollapse="childIsCollapse"></MenuItem>
			</a-sub-menu>
			<a-menu-item key="5" :key="child.name" v-else v-show="!child.meta.isHide">
				<a-icon :type="child.meta.icon" :style="child.meta.left?'margin-left:12px':''"/>
				<span>{{ child.meta.title }}</span>
			</a-menu-item>
		</a-menu>
	</div>
